...
def main(args):
    ...
    opts = StateDictOptions(full_state_dict=True, cpu_offload=True)       # put result on CPU
    model_state_dict = get_model_state_dict(model, options=opts)
    
    if torch.distributed.get_rank() == 0:
        torch.save(model_state_dict, "fsdp-multigpu-simple-model_full.pt")
