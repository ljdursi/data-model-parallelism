...
# DONE add os import so we can get at environment variables w/ os.environ.get()
import os
...
# DONE add torch DDP import - import as DDP
from torch.nn.parallel import DistributedDataParallel as DDP

# DONE import torch distributed as dist for barrier, process_group operations, reductions
...
def main(args):
    # define torch device
    # DONE Step 1 - get LOCAL_RANK with os.environ.get to set the GPU appropriately
    local_rank = int(os.environ.get("LOCAL_RANK", default="0"))
    device = torch.device(f"cuda:{local_rank}" if torch.cuda.is_available() else "cpu")

    # DONE Step 1 - set up process group
    ...
    # DONE only need to download data once per node but everyone needs to know information about the dataset
    # It will probably be easiest to have a barrier involved
    if local_rank == 0:
    ...
    # DONE define train_sampler and test_sampler from tud.distributed.DistributedSampler
    ...
    # DONE - optional wrap model with nn.SyncBatchNorm.convert_sync_batchnorm to sync the batch norms across microbatches
    net = nn.SyncBatchNorm.convert_sync_batchnorm(net)
    # DONE - wrap model with nn.parallel.DistributedDataParallel.   You'll have to provide device_ids=[local_rank])
    ...
    # DONE step 1 only need to see this once - just have global rank == 0 print this
    if global_rank == 0:
        print(f"Beginning training: {args.epochs} epochs")

    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                 on_trace_ready=torch.profiler.tensorboard_trace_handler('/workspace/logs/eurosat_pipeline_step2'),
        ...
        for epoch in range(args.epochs):
            ...
            # DONE - set the epoch in train_sampler so that the seed is different each time
            ...
            # DONE step 1 - synchronize with barrier at the end of each epoch, or else 
            # stats will be wrong 
            dist.barrier()
            ...
            # DONE Step 2 - for v_accuracy, f_loss, these are already tensors, and we can use
            ...
            # DONE step 1 Just have rank 0 print this out
            if global_rank == 0:
                print(
    ...
    # DONE step 1- we don't need to have all ranks print this
    if global_rank == 0:
        # we don't need to have all ranks print finished training
        print("Finished Training")
    
        # DONE step 1 - we should only have one rank save the model
        save_path = "./eurosat_net.pth"
        torch.save(net.state_dict(), save_path)
    
    # DONE step 1- get rid of the process group
